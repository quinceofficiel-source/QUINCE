...
export function AddressSearch({
  ...
  return (
    <div ref={boxRef} className="relative">
      <div className={cn("flex items-stretch gap-2", large ? "flex-col sm:flex-row" : "flex-row")}>
        <div
          className={cn(
            "relative flex min-w-0 flex-1 items-center rounded-2xl border border-line bg-white",
            large ? "h-14" : "h-12",
          )}
        >
        ...
        <button
          ...
          className={cn(
            "inline-flex items-center justify-center gap-2 rounded-2xl bg-ink font-medium text-white hover:bg-neutral-800",
            large ? "h-14 px-6" : "h-12 px-4 text-sm",
          )}
        >
